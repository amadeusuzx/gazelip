...
def get(raw_array, top_flag, stat_flag, lip_rect,i):
    ...
    while True:
        ...
        if stat_flag.value:
            np.copyto(X_1[top_flag.value % 100], frame_copy)
            top_flag.value += 1
            if stat_flag.value == 2:
                cv2.rectangle(
                    frame, (lip_rect[0] - lip_rect[2]//2, lip_rect[1] - lip_rect[3]//2), (lip_rect[0] + lip_rect[2]//2, lip_rect[1] + lip_rect[3]//2), (0, 0, 255), 2)
        # background[480:960,960:1600,:] = frame[:,:,:]
        cv2.putText(frame,origin_commands[i.value],(300,50),cv2.FONT_HERSHEY_SIMPLEX,1,(255,0,0),2,cv2.LINE_AA)
